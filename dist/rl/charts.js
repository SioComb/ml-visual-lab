export function chart(rows, key, title, average = false, xLabel = 'Episode') {
  const w = 480,
    ...
    bottom = 35;
  if (!rows.length)
    return `<div class="rl-empty">${title} · 実行すると推移を表示します</div>`;
  ...
  const points = values
    .map((_, i) => i)
    .filter((i) => i % stride === 0 || i === rows.length - 1);
  const path = (series) =>
    points
      .map(
        (i, j) =>
          `${j ? 'L' : 'M'}${x(i).toFixed(1)},${y(series[i]).toFixed(1)}`,
      )
      .join(' ');
  let sum = 0;
  const moving = values.map((v, i) => {
    sum += v;
    if (i >= 20) sum -= values[i - 20];
    return sum / Math.min(i + 1, 20);
  });
  const ticks = [lo, (lo + hi) / 2, hi]
    .map(
      (v) =>
        `<line x1="${left}" x2="${w - right}" y1="${y(v)}" y2="${y(v)}" stroke="#e3e9e4"/><text x="${left - 7}" y="${y(v) + 4}" text-anchor="end">${v.toFixed(1)}</text>`,
    )
    .join('');
