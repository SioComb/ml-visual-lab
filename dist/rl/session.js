...
export function environment(kind, config, rng) {
  return kind === 'maze' ? new Maze(config.preset, config.seed) : new Snake(rng);
}
export class TrainingSession {
  ...
  step() {
    if (this.finished) { this.env.reset(); this.steps = 0; this.reward = 0; this.finished = false; }
    ...
    const update = this.learner.update({ state, action: selected.action, ...transition, nextState: this.env.state() });
    this.last = { ...selected, ...transition, ...update };
    if (transition.done || this.steps >= this.config.maxSteps) {
      this.finished = true;
      this.history.push({ reward: this.reward, steps: this.steps, score: this.env.score ?? 0, outcome: transition.done ? transition.outcome : 'step上限' });
    }
  }
  snapshot() {
    return { table: this.learner.table, history: this.history, env: this.env.snapshot(), steps: this.steps, reward: this.reward, last: this.last, finished: this.finished };
  }
}
