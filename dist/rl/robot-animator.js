...
export function createRobotAnimator() {
  ...
  const paint = (frame, fx) => {
    ...
    stage.innerHTML =
      ((fx && fx.back) || '') +
      robotMarkup(frame, project) +
      ((fx && fx.front) || '');
  };
  const halt = () => {
    if (handle) dropFrame(handle);
    handle = 0;
    queue = [];
  };
  ...
  function settled(seg) {
    if (seg.type === 'walk')
      return idlePose({ x: seg.to.x, y: seg.to.y }, seg.to.yaw);
  ...
  function tick(stamp) {
    ...
    else if (seg.type === 'celebrate')
      frame = celebratePose(seg.pos, seg.yaw, p);
    else if (seg.type === 'electrocute') {
      frame = electrocutePose(seg.pos, seg.yaw, p, seg.calm);
      fx = electrocuteFx(seg, p, project);
  ...
  function sync(opts) {
        ...
        queue = [
          {
            type: 'bump',
            pos: spot,
            fromYaw: pose ? pose.yaw : 0,
            toYaw: headingForAction(action),
            dur: 260,
          },
        ];
        run();
    ...
    const dx = (state % 5) - (tile % 5),
      dy = Math.floor(state / 5) - Math.floor(tile / 5);
    ...
    queue = [
      { type: 'walk', from, to, dur: Math.abs(swing) > 0.35 ? 380 : 300 },
    ];
    if (arrival === 'G')
      queue.push({ type: 'celebrate', pos: spot, yaw: to.yaw, dur: 480 });
    else if (arrival === 'T')
      queue.push({
        type: 'electrocute',
        pos: spot,
