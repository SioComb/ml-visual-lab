...
export function project(x, y, z = 0) {
  const depth = 14 - (y - 2.5) * 0.572 - z * 0.82;
  return [
    350 + ((x - 2.5) * 1330) / depth,
    310 + (((y - 2.5) * 0.82 - z * 0.572) * 1330) / depth,
  ];
}
...
const points = (vertices) =>
  vertices.map((p) => p.map((v) => v.toFixed(2)).join(',')).join(' ');
export const face = (vertices, fill, extra = '') =>
...
export function roundedFace(vertices, fill, extra = '') {
  const ends = vertices.map((p, i) => {
    ...
    const toward = (target) => {
      ...
      const ratio = Math.min(4 / length, 0.15);
      return [
...
export function block(
  ...
  const b = corners(x, y, width, length, bottom),
    t = corners(x, y, width, length, top);
  return (
    face([t[0], t[3], b[3], b[0]], side) +
    face([t[1], t[2], b[2], b[1]], side) +
    face([t[3], t[2], b[2], b[3]], front) +
    roundedFace(
      t,
      surface,
      `stroke="${bevel}" stroke-width="1.4" stroke-linejoin="round"`,
    )
  );
}
...
export function tray(frame, frameFront) {
  return (
    ...
    block(-0.23, -0.23, 5.46, 0.18, -0.08, 0.19, frame, '#81988a', '#779386')
  );
}
export function trayRims(frame) {
  return (
    block(-0.23, -0.05, 0.18, 5.28, -0.08, 0.19, frame, '#7c9f90', '#668b7c') +
    block(5.05, -0.05, 0.18, 5.28, -0.08, 0.19, frame, '#7c9f90', '#668b7c') +
    block(-0.23, 5.05, 5.46, 0.18, -0.08, 0.19, frame, '#779c89', '#668b7c')
  );
}
