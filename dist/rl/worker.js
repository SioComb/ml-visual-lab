...
let session,
  ...
  speed = 10;
function report() {
  self.postMessage({ snapshot: session.snapshot(), running });
}
function tick() {
  if (!running) return;
  try {
    ...
    do {
      session.step();
    } while (session.history.length < batch && performance.now() < end);
...
self.onmessage = ({ data }) => {
  try {
    ...
    running = false;
    if (data.type === 'init')
      session = new TrainingSession(data.kind, data.config);
    if (data.type === 'train') {
