...
export class Bandit {
  constructor({
    ...
    this.rewards = Array.from({ length: arms }, (_, i) =>
      random(seed + 100 + i),
    );
  ...
  explorationBonus(i) {
    return this.counts[i] === 0
      ? Infinity
      : Math.sqrt((2 * Math.log(this.history.length)) / this.counts[i]);
  }
  step() {
    ...
    if (this.algorithm === 'ucb') {
      ...
      selected =
        untried >= 0
          ? { action: untried, exploring: true }
          : choose(
              this.q.map(
                (q, i) => q + Math.sqrt((2 * Math.log(t)) / this.counts[i]),
              ),
              0,
              this.rng,
            );
      ...
      selected = choose(
        this.q,
        this.algorithm === 'greedy' ? 0 : this.epsilon,
        this.rng,
      );
      selected.reason = selected.exploring
        ? '探索 Exploration · ランダムに選択'
        : '活用 Exploitation · Q値が最大のアーム';
    }
    const a = selected.action,
      reward = Number(this.rewards[a]() < this.probabilities[a]);
    this.counts[a]++;
