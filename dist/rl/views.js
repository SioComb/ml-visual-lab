...
export function banditView(bandit, reveal) {
  const ucb = bandit.algorithm === 'ucb',
    trials = bandit.history.length;
  const fmt = (v) =>
    v === Infinity ? '∞' : Number.isFinite(v) ? v.toFixed(3) : '—';
  ...
  if (ucb && trials > 0) {
    ...
    topArm =
      untried >= 0
        ? untried
        : bandit.q
            .map((q, i) => q + bandit.explorationBonus(i))
            .reduce((best, s, i, all) => (s > all[best] ? i : best), 0);
  }
  return `<div class="rl-slots">${bandit.q
    .map((q, i) => {
      const bonus = bandit.explorationBonus(i),
        score = q + bonus;
      const ucbRows = ucb
...
export function qDetails(table, selected, cells, last) {
  const values = table[selected] ?? [0, 0, 0, 0],
    best = bestActions(values);
...
export function learningCharts(history, kind) {
  return (
    chart(history, 'reward', 'Episodeごとの累積報酬', true) +
    chart(
      history,
      'steps',
      kind === 'maze' ? 'Episodeごとのstep数' : '生存step数',
      true,
    ) +
    (kind === 'snake' ? chart(history, 'score', 'Scoreと平均Score', true) : '')
  );
}
