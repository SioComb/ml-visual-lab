...
const numberField = (id, label, value, min, max, step = 1) =>
  `<label for="rl-${id}">${label}</label><input id="rl-${id}" type="number" value="${value}" min="${min}" max="${max}" step="${step}" required>`;
const selectField = (id, label, entries) =>
  `<label for="rl-${id}">${label}</label><select id="rl-${id}">${Object.entries(
    entries,
  )
...
const BANDIT_INTRO = {
  greedy:
    '現在もっとも推定報酬 Q(a) が高いアームだけを選び続けます。ランダムな探索は行わないため、最初の数回で偶然よい結果が出たアームに偏り、本当はもっと良いアームを見逃すことがあります。',
  epsilon:
    'εの確率でランダムなアームを試し（探索）、それ以外はQ(a)が最大のアームを選びます（活用）。εを0から0.3へ動かし、同じSeedで比較してみましょう。',
...
const BANDIT_READING = {
  greedy:
    'Greedyは現在もっとも良さそうなアームだけを選びます。そのため、初期の偶然によって評価が低くなったアームは、その後ほとんど試されません。「確率をシャッフル」して何度か実行し、ε-GreedyやUCBと結果を比較してみましょう。',
  epsilon:
    'ε=0では最初の偶然に引っぱられがちです。εを増やすと不利なアームも試せますが、その場の報酬は減ります。同値の最大Q値はランダムに選びます。',
...
const BANDIT_FORMULA = {
  ...
  epsilon:
    '確率 ε：ランダムに選択 ／ 確率 1−ε：a = argmax Q(a)\nQ(a) ← Q(a) + [Reward − Q(a)] ÷ N(a)',
...
export function initReinforcement(onLessonChange) {
  ...
  function settings() {
    ...
    if (kind === 'maze')
      html +=
        selectField('preset', '迷路プリセット', {
          ...Object.fromEntries(
            Object.entries(MAZES).map(([k, v]) => [k, v.name]),
          ),
    ...
    html += selectField(
      ...
      kind === 'bandit' || kind === 'ads'
        ...
        : { 1: '1 Episode', 10: '10 Episodes', 50: '50 Episodes' },
    );
    ...
    if (kind === 'bandit') bandit = new Bandit(config);
      ...
      try {
        ...
        worker.onmessage = ({ data }) => {
          ...
          if (data.error) {
            stop();
            $('rl-status').textContent = '学習エラー：' + data.error;
            return;
          }
          snapshot = data.snapshot;
          if (pendingPlay && !data.running) {
            pendingPlay = false;
            startPlay();
            return;
          }
        ...
        worker.onerror = () => {
          ...
          $('rl-status').textContent =
            'Workerを開始できません。HTTP経由で開き直してください。';
        };
        ...
        render();
        $('rl-status').textContent =
          'Workerに対応したブラウザでHTTP経由で開いてください';
        return;
      }
    }
  ...
  function startPlay() {
    ...
    evaluation = {
      env: environment(kind, config, random(config.seed + 1000)),
      learner,
      rng: random(config.seed + 2000),
      steps: 0,
      reward: 0,
      last: null,
      done: false,
    ...
    running = true;
    $('rl-status').textContent = 'Evaluation · ε=0 / 更新なし';
    render();
    timer = setTimeout(playTick, 350);
  }
  function playTick() {
    if (!running || !evaluation) return;
    const e = evaluation,
      selectedAction = e.learner.select(e.env.state(), 0, e.rng);
    const transition = e.env.step(selectedAction.action);
    e.steps++;
    e.reward += transition.reward;
    e.last = { ...selectedAction, ...transition };
    e.done = transition.done || e.steps >= config.maxSteps;
    if (e.done) {
      running = false;
      $('rl-status').textContent = transition.done
        ? transition.outcome
        : '再生終了 · step上限（未到達）';
    }
    render();
  ...
  function render() {
    ...
    if (isAds) {
      ...
      $('rl-reading').textContent =
        '推定CTRが少し低い広告でも、分布から大きな値を引けば選ばれます。未知の広告を試す機会と、有望な広告を使う機会を、サンプリングで両立します。εのような探索率は設定しません。広告文や画像の内容は学習していません。';
    ...
    const last = isBandit
      ...
      : evaluation
        ? evaluation.last
        : snapshot.last;
    $('rl-action').textContent = last
      ? `${isBandit ? 'アーム ' + String.fromCharCode(65 + last.action) : '行動 ' + DIRECTIONS[last.action]} · ${last.reason ?? (last.exploring ? '探索 Exploration' : '活用 Exploitation')} · Reward ${last.reward.toFixed(1)}`
      : evaluation
        ? '学習済みのQ値で、最初の行動を選びます。'
        : 'まずは1ステップずつ動かして、最初の行動を見てみましょう。';
    if (isBandit) {
      $('rl-metrics').innerHTML =
        metric('試行', bandit.history.length, `/ ${config.limit}`) +
        metric('累積報酬', bandit.total) +
        metric(
          '平均報酬',
          (bandit.total / (bandit.history.length || 1)).toFixed(3),
        );
      $('rl-board').innerHTML = banditView(bandit, $('rl-reveal').checked);
      $('rl-explanation').textContent =
        BANDIT_INTRO[config.algorithm] || BANDIT_INTRO.epsilon;
      $('rl-reading').textContent =
        BANDIT_READING[config.algorithm] || BANDIT_READING.epsilon;
      $('rl-detail').hidden = true;
      $('rl-legend').textContent =
        '当たり +1 / はずれ 0。緑の枠が直近に選ばれたアームです。';
      $('rl-charts').innerHTML =
        chart(bandit.history, 'reward', '累積報酬', false, '試行') +
        chart(bandit.history, 'average', '平均報酬', false, '試行');
      $('rl-progress').textContent =
        '設定を変える前に結果を記録し、同じSeed・スロット数・試行数で比較できます。';
      $('rl-comparison').hidden = false;
      $('rl-comparison').innerHTML =
        `<button class="quiet" id="rl-save" ${bandit.history.length ? '' : 'disabled'}>現在の比較結果を記録</button>${comparisons.length ? '<div class="rl-table-scroll"><table><thead><tr><th>手法</th><th>ε</th><th>Seed / 台数</th><th>試行</th><th>累積報酬</th><th>平均報酬</th></tr></thead><tbody>' + comparisons.map((c) => `<tr><td>${c.name}</td><td>${c.epsilon}</td><td>${c.seed} / ${c.arms}</td><td>${c.trials}</td><td>${c.total}</td><td>${c.average}</td></tr>`).join('') + '</tbody></table></div>' : ''}`;
      $('rl-save').onclick = () => {
        comparisons.push({
          name: BANDIT_NAMES[config.algorithm],
          epsilon: config.algorithm === 'epsilon' ? config.epsilon : '—',
          seed: config.seed,
          arms: config.arms,
          trials: bandit.history.length,
          total: bandit.total,
          average: (bandit.total / bandit.history.length).toFixed(3),
        });
        comparisons = comparisons.slice(-6);
        render();
      };
      $('rl-update').textContent =
        BANDIT_FORMULA[config.algorithm] || BANDIT_FORMULA.epsilon;
    } else {
      const env = evaluation?.env.snapshot() ?? snapshot.env;
      const steps = evaluation?.steps ?? snapshot.steps,
        reward = evaluation?.reward ?? snapshot.reward;
      $('rl-metrics').innerHTML =
        metric('完了Episode', snapshot.history.length, `/ ${config.limit}`) +
        metric(
          evaluation ? '再生の累積報酬' : '現在の累積報酬',
          reward.toFixed(1),
          `${steps} steps`,
        ) +
        (kind === 'snake'
          ? metric('Score', env.score, `ε = ${evaluation ? 0 : config.epsilon}`)
          : metric(
              '探索率 ε',
              evaluation ? 0 : config.epsilon,
              evaluation ? 'Q-tableを更新しません' : '学習中は一定',
            ));
      $('rl-detail').hidden = false;
      if (kind === 'maze') {
        $('rl-board').innerHTML = mazeView(
          env,
          snapshot.table,
          selected,
          $('rl-overlay').checked,
        ...
        $('rl-legend').textContent =
          'S: Start / ◎: Goal +10 / ■: Wall / ⚠: Trap −10 / 🤖: Agent。通常移動・壁への試行 −0.1。矢印は現在のQ値に基づく方策です。';
      } else {
        $('rl-board').innerHTML = snakeView(env);
        $('rl-detail').innerHTML =
          `<h3>なぜEpisodeを増やしても成績が伸びない？</h3><p>Episodeを増やせば、Q値の推定は少しずつ安定していきます。一方で、ヘビの体の形や細かな位置関係は状態に含まれていません。そのため、Q-tableが十分に学習された後は、Episodeを増やしても大きな改善が起こりにくくなります。</p><p class="field-help">状態数の内訳：危険4方向（各「危険 / 安全」→ 2⁴ = 16）× りんごの相対方向（横3 × 縦3 = 9）× 進行方向（4）＝ 576</p><p>学習した状態：<strong>${Object.keys(snapshot.table).length}</strong> / 576</p>`;
        $('rl-legend').textContent =
          '● りんご +10 / 壁・体への衝突 −10 / 通常移動 −0.1。りんごを取ると1マス伸びます。即時の逆向き移動も体への衝突です。';
      }
      $('rl-charts').innerHTML = learningCharts(snapshot.history, kind);
      $('rl-progress').textContent =
        '完了した学習Episodeを表示。上限で打ち切った回も含みます。Evaluationは学習グラフに含めません。';
      $('rl-comparison').hidden = true;
    ...
    if (focusedState !== undefined)
      $('rl-board')
        .querySelector(`[data-state="${focusedState}"]`)
  ...
  $('rl-pause').onclick = () => {
    stop();
    $('rl-status').textContent = '一時停止';
    render();
  };
  $('rl-play').onclick = () => {
    if (evaluation && !evaluation.done) {
      running = true;
      $('rl-status').textContent = 'Evaluation · ε=0 / 更新なし';
      playTick();
      return;
  ...
  $('rl-board').onclick = (event) => {
    ...
    if (cell) {
      selected = Number(cell.dataset.state);
      render();
    }
  };
