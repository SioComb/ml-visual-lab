...
export function normalizeTransactions(rows) {
  ...
  const transactions = rows.map((row) => {
    if (!Array.isArray(row) || row.some((item) => typeof item !== 'string'))
      throw new Error('商品名は文字列で指定してください。');
    const basket = ordered(
      new Set(row.map((item) => item.trim()).filter(Boolean)),
    );
    if (!basket.length) throw new Error('商品がない購買行があります。');
    for (const item of basket) {
      if (item.length > 80)
        throw new Error('商品名は80文字以内で指定してください。');
      items.add(item);
    }
    return basket;
  });
  if (items.size > LIMITS.items)
    throw new Error('商品は32種類以内で指定してください。');
  return transactions;
}

export function mine(
  rows,
  { algorithm = 'apriori', support = 0.2, maxLength = 3 } = {},
) {
  if (!['apriori', 'eclat', 'fpgrowth'].includes(algorithm))
    throw new Error('未知のアルゴリズムです。');
  if (!Number.isFinite(support) || support <= 0 || support > 1)
    throw new Error('支持度は0より大きく100%以下で指定してください。');
  if (
    !Number.isInteger(maxLength) ||
    maxLength < 1 ||
    maxLength > LIMITS.length
  )
    throw new Error('最大サイズは1〜5で指定してください。');
  ...
  function guard() {
    if (
      ++operations > 15000000 ||
      (operations % 1024 === 0 && performance.now() - start > 8000)
    )
      throw new Error(
        '探索量が上限に達しました。支持度を上げるか、最大サイズ・データ件数を減らしてください。',
      );
  }
  function add(items, count) {
    if (patterns.length >= LIMITS.patterns)
      throw new Error(
        '頻出組合せが20,000件を超えました。支持度を上げるか最大サイズを減らしてください。',
      );
    patterns.push({
      items: ordered(items),
      count,
  ...
  for (const basket of transactions)
    for (const item of basket) counts.set(item, (counts.get(item) || 0) + 1);
  stats.scans++;
  const frequent = ordered(
    [...counts.keys()].filter((item) => counts.get(item) >= minimum),
  );
  ...
  if (algorithm === 'apriori') {
    ...
    for (let size = 2; size <= maxLength && level.length; size++) {
      ...
      for (const basket of baskets)
        for (let index = 0; index < candidates.length; index++) {
          guard();
          if (candidates[index].every((item) => basket.has(item)))
            candidateCounts[index]++;
        }
      for (let index = 0; index < candidates.length; index++) {
        const candidate = candidates[index],
          count = candidateCounts[index];
        if (count >= minimum) {
          next.push(candidate);
          add(candidate, count);
        }
    ...
    transactions.forEach((basket, tid) =>
      basket.forEach((item) => vertical.get(item)?.add(tid)),
    );
    stats.scans++;
    for (const [item, tids] of vertical)
      trace.push({ item, count: tids.size, tids: [...tids].slice(0, 16) });
    function visit(prefix, extensions) {
      for (let i = 0; i < extensions.length; i++) {
        guard();
        const [item, tids] = extensions[i],
          pattern = [...prefix, item];
        add(pattern, tids.size);
        if (pattern.length >= maxLength) continue;
        const next = [];
        for (let j = i + 1; j < extensions.length; j++) {
          const [other, ids] = extensions[j],
            intersection = new Set();
          stats.intersections++;
          const [small, big] = tids.size < ids.size ? [tids, ids] : [ids, tids];
          for (const id of small) {
            guard();
            if (big.has(id)) intersection.add(id);
          }
      ...
      if (!knownCounts)
        for (const { items, weight } of paths)
          for (const item of items) {
            guard();
            frequencies.set(item, (frequencies.get(item) || 0) + weight);
          }
      const header = new Map(
        [...frequencies]
          .filter(([, count]) => count >= minimum)
          .map(([item, count]) => [item, { count, nodes: [] }]),
      );
      const root = { item: null, children: new Map(), parent: null, count: 0 };
      for (const { items, weight } of paths) {
        const sorted = items
          .filter((item) => header.has(item))
          .sort(
            (a, b) =>
        ...
        for (const item of sorted) {
          ...
          if (!node.children.has(item)) {
            ...
            header.get(item).nodes.push(child);
            stats.nodes++;
          }
    ...
    const tree = buildTree(
      transactions.map((items) => ({ items, weight: 1 })),
      counts,
    );
    stats.scans++;
    function preview(node, depth) {
      for (const child of node.children.values()) {
        ...
        trace.push({ item: child.item, count: child.count, depth });
        preview(child, depth + 1);
      }
    }
    preview(tree.root, 0);
    function grow({ header }, suffix) {
      for (const [item, entry] of [...header].sort(
        (a, b) => a[1].count - b[1].count,
      )) {
        guard();
        const pattern = [item, ...suffix];
        add(pattern, entry.count);
        if (pattern.length >= maxLength) continue;
        const paths = [];
  ...
  patterns.sort(
    (a, b) =>
      ...
      key(a.items).localeCompare(key(b.items)),
  );
  return {
    algorithm,
    minimum,
...
export function associationRules(patterns, minConfidence = 0.6) {
  if (!Number.isFinite(minConfidence) || minConfidence < 0 || minConfidence > 1)
    throw new Error('確信度が無効です。');
  const lookup = new Map(
    patterns.map((pattern) => [key(pattern.items), pattern]),
  );
  const rules = [];
  ...
  for (const pattern of patterns) {
    ...
    for (const consequent of pattern.items) {
      ...
      const left = lookup.get(key(antecedent)),
        right = lookup.get(key([consequent]));
